...
class TalkingHeadClient:
    ...
    def upload_file(self, file_path: str) -> str:
        ...
        Returns:
            URL string for the uploaded file, or empty string on failure
        """
        if not self.api_key:
        ...
        try:
            ...
            print(f"[TalkingHead] Uploading file: {file_path} ({file_size} bytes)")
    ...
    def generate_talking_video(
        ...
        Args:
            image_path_or_url: Local path or URL of the face image
            audio_path_or_url: Local path or URL of the TTS audio file
            duration: Expected duration in seconds

        Returns:
            URL of the generated video, or empty string on failure
        """
        ...
        try:
            # Step 1: Upload local files to get URLs
            image_url = image_path_or_url
            if os.path.exists(image_path_or_url):
                print(f"[TalkingHead] Uploading local image...")
            ...
            if not task_id:
                print(f"[TalkingHead] No task ID in response")
                return ""
            ...
            print(f"[TalkingHead] Talking video generation failed: {type(e).__name__}: {e}")
            return ""
