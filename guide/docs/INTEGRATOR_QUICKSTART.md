...
| 流水线 `pipeline_key` | 适用场景 | 动效交付 |
|----------------------|----------|---------|
| `template_editor` / `standard` | **默认导购成片** | FFmpeg：ASS 字幕（含 `hf-caption-*` 降级映射）、xfade 转场、`hf_overlays` 滤镜 |
| `hyperframes_template` | 调试 / 完整 GSAP 实验 | HyperFrames HTML 全量渲染（跳过场景生成，非导购主路径） |
