...
async function waitForHfPreviewReady(page: import('@playwright/test').Page) {
  await expect.poll(async () => {
    ...
    return iframe.evaluate((el: HTMLIFrameElement) => {
      ...
      const hasPop = Boolean(el.contentDocument?.querySelector('[data-hf-component="caption-pop-bounce"]'));
      return Number.isFinite(duration) && duration > 0 && hasPop;
    }).catch(() => false);
  }, { timeout: 35_000 }).toBeTruthy();
}

test('editor previews hf-caption-pop bounce subtitles', async ({ page, request }) => {
  test.setTimeout(60_000);

  const created = await request.post(`${apiBase}/api/templates`, {
    data: {
      name: `E2E HF pop ${Date.now()}`,
      type: 'e2e',
      description: 'hf-caption-pop preview',
    },
  });
  expect(created.ok(), await created.text()).toBeTruthy();
  const template = await created.json() as { id: string; dsl_json: { segments: Array<Record<string, unknown>> } };

  const narrationText = '弹跳字幕预览测试';
  ...
  await page.getByTestId('subtitle-style-card-hf-caption-pop').click();
