...
品牌导购短视频：**模板编辑 + 数字人 + 渲染流水线**。对外由 FastAPI 网关（`:8000`）统一代理导购 API 与静态资源。
...
| `worker/` | 渲染流水线：KIE 场景融合 → MOSI 配音 → 口型视频 → FFmpeg |
...
- **导购前端**：http://127.0.0.1:5173
- **集成 Playground**：http://127.0.0.1:5173/debug
...
```bash
cd guide
make test-guide-server   # Express API 单测
